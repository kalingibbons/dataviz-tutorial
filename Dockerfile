...
# and `pip install dataviz_tutorial` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "dataviz_tutorial", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n dataviz_tutorial -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "dataviz_tutorial", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "dataviz_tutorial", "YOUR_CONSOLE_SCRIPT"]
